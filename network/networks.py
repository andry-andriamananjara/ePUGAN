...
'''
The below codes are taken from
Haolin Liu. Pytorch unoffical implementation of point cloud upsampling adversarial
network. https://github.com/HaolinLiu97/PUGAN-pytorch
'''
...
class up_projection_unit(nn.Module):
    def __init__(self,params=None,up_ratio=4):
        ...
        self.conv2=nn.Sequential(
            Conv1d(in_channels=128, out_channels=128,kernel_size=1),
            nn.ReLU()
        )
    ...
    def forward(self,input):
        #print('up_projection XXXX : ', self.params['scale'])
        if self.params['scale']=='arbitrary':
            #print(input.shape)
            L  = self.conv1(input)#b,128,4*n
            L0 = self.down_block(L)#b,128,n
            H1 = self.up_block2(L0)#b,128,4*n
            E0 = L-H1
            E0 = self.conv2(E0)
            H2 = L+E0
...
class Discriminator(nn.Module):
    def __init__(self,params,in_channels):
        ...
        elif self.params['dis_attention']=='mamba2':
            self.ssm_mamba=ssm_conv(dim  = self.start_number*4).cuda()
        else:
    ...
    def forward(self,inputs):
        ...
        if self.params['dis_attention']=='mamba':
            ...
            features=features.permute(0,2,1)
            features=self.ssm_mamba(features)
            features=features.permute(0,2,1)
        else:

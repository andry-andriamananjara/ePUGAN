...
class PUNET_Dataset_Whole(data.Dataset):
    #def __init__(self, data_dir='../MC_5k/GT',n_input=1024):
    def __init__(self, data_dir='../MC_5k',n_input=1024):
        ...
        self.names = [x.split('.')[0] for x in file_list if isfile(join(data_dir, x))]
        self.sample_path = [os.path.join(data_dir, x) for x in file_list if isfile(join(data_dir, x))]
    ...
    def __getitem__(self, index):
        ...
        gt     = np.loadtxt(self.sample_path[index])

        centroid = np.mean(gt[:, :3], axis=0)
        #furthest_distance = np.amax(np.sqrt(np.sum((gt[:, :3] - centroid) ** 2, axis=-1)), axis=0)
        furthest_distance  = np.max(np.linalg.norm(gt[:,:3]-centroid,axis=1))
        radius = furthest_distance  # not very sure?
        #print(radius)
        
        #normalization
        gt     = (gt-centroid)/furthest_distance
        
        #Random index
        self.raw_input_points = gt.shape[0]
        random_index = utils.nonuniform_sampling(self.raw_input_points, sample_num=self.n_input)
        inputs = gt[random_index][:,0:3]
        ...
        return inputs, gt, radius #normalized_points,furthest_dist,centroid


#PU NET uniform
class PUNET_PU1K_Dataset(data.Dataset):
    def __init__(self, skip_rate=1, npoint=1024
            , dataname = "punet"
            , uniform=True, use_random=True, use_norm=True, isTrain=True):
        super().__init__()
        ...
        print('Dataset :: ', self.dataname,' Uniform : ', uniform,' Train : ',isTrain)
        
        if self.dataname=='punet':

            if self.uniform:
                gt_h5_file_path = "../MC_5k/Mydataset/PUNET/knn_uniform_1024_4096_train.h5"
                gt_h5_file      = h5py.File(gt_h5_file_path)
                self.gt         = gt_h5_file['gt'][:]
                self.input      = gt_h5_file['input'][:]
            else:
                gt_h5_file_path = "../MC_5k/Mydataset/PUNET/knn_non_uniform_1024_4096_train.h5"
                gt_h5_file      = h5py.File(gt_h5_file_path)
                self.gt         = gt_h5_file['gt'][:]
                self.input      = gt_h5_file['input'][:]
            self.npoint         = self.input.shape[1]
            print(gt_h5_file_path)
            print(' :: GT ',self.gt.shape,' :: INPUT ',self.input.shape,' Npoint :: ',self.npoint)

        elif self.dataname=='pugan':

            if self.uniform:
                gt_h5_file_path = "../MC_5k/Mydataset/PUGAN/PUGAN_100K_uniform_256_1024_train.h5"
                gt_h5_file      = h5py.File(gt_h5_file_path)
                self.gt         = gt_h5_file['gt'][:]
                self.input      = gt_h5_file['input'][:]
            else:
                gt_h5_file_path = "../MC_5k/Mydataset/PUGAN/PUGAN_100K_non_uniform_256_1024_train.h5"
                gt_h5_file      = h5py.File(gt_h5_file_path)
                self.gt         = gt_h5_file['gt'][:]
                self.input      = gt_h5_file['input'][:]
            self.npoint         = self.input.shape[1]
        ...
        elif self.dataname=='pu1k_scale':
            #pu1k_scale
            gt_h5_file_path = "../MC_5k/Mydataset/PU1K/non_uniform/train/pu1k_poisson_256_poisson_1024_pc_2500_patch50_addpugan.h5"
            gt_h5_file      = h5py.File(gt_h5_file_path)
            self.input      = gt_h5_file['poisson_1024'][:]
            self.gt         = gt_h5_file['poisson_1024'][:]
            self.npoint     = self.input.shape[1]# need to remove
            
        else:
            #pu1k
            gt_h5_file_path = "../MC_5k/Mydataset/PU1K/non_uniform/train/pu1k_poisson_256_poisson_1024_pc_2500_patch50_addpugan.h5"
            gt_h5_file      = h5py.File(gt_h5_file_path)
            self.npoint     = gt_h5_file['poisson_256'][:].shape[1]
            if self.uniform:
                self.input = gt_h5_file['poisson_256'][:]
                self.gt    = gt_h5_file['poisson_1024'][:]
            else:
                self.input = gt_h5_file['poisson_1024'][:]
                self.gt    = gt_h5_file['poisson_1024'][:]
        ...
        self.data_npoint  = self.gt.shape[1]
    ...
    def __getitem__(self, index):
        '''
        select random point in each data according to npoint=1024
        '''
        ...
        if self.use_norm:
            ...
            if np.random.rand() > 0.5:
                input_data = utils.jitter_perturbation_point_cloud(input_data, sigma=0.001, clip=0.03)
...
class PUNET_Dataset(data.Dataset):
    ...
    def __init__(self, h5_file_path='../MC_5k/Mydataset/PUNET/PUNET_train_Patches_noHole_and_collected.h5',split_dir='./all_PUNET.txt',
    ...
    #             skip_rate=1, npoint=1024, use_random=True, use_norm=True,isTrain=True):
        ...
        #print('XXX ',h5_file['poisson_4096'])
...
if __name__=="__main__":
    ...
    #    'GT      :: ',gt_data.shape,
    #    'Radius  :: ',radius_data.shape
    #)
    
    dataset=PUNET_PU1K_Dataset(uniform=False, dataname = "pu1k_scale")
    input_data,gt_data,radius_data=dataset.__getitem__(0)
    print(
        'Input   :: ', input_data.shape,
        'GT      :: ', gt_data.shape,

...
def predict_from_h5():
    ...
    #for epoch in [49]:
    for epoch in [19, 29, 39, 49, 59, 69, 79, 89, 99]:

        # Check if the epochs exist
        ckpt_model = resume
        print(ckpt_model)
        
        if ckpt_model.find(str(epoch))!=-1:
            ...
            for path in [50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 550, 600, 650, 700, 750, 800]:
            #for path in tqdm(range(len(input_h5))):
                ...
                if params['scale']=='arbitrary':
                    ...
                    plot_save_pcd(pred_pc_fpsMI, resume.split('/')[-2]+str('.pth').replace('.','_'+str(path)+'_'+str(non_uniform)+'_PRED_MI_'+str(epoch)+'.'), exp_name)#prediction
                    plot_save_pcd(pred_pc_fps, resume.split('/')[-2]+str('.pth').replace('.','_'+str(path)+'_'+str(non_uniform)+'_PRED_Doub'+str(epoch)+'.'), exp_name)#prediction
                    plot_save_pcd(inputMI, resume.split('/')[-2]+str('.pth').replace('.','_'+str(path)+'_'+str(non_uniform)+'_INPUT_MI_'+str(epoch)+'.'), exp_name)#prediction
                    plot_save_pcd(gt, resume.split('/')[-2]+str('.pth').replace('.','_'+str(path)+'_'+str(non_uniform)+'_GT_'+str(epoch)+'.'), exp_name)#prediction
                else:
                    input = torch.nan_to_num(input, nan=0.0)
                    input, centroid, furthest_distance = pc_normalization(input)
                    # Patch prediction
                    ...
                    gt      = gt[0, ...]
                    gt      = gt.detach().cpu().numpy()
                    plot_save_pcd(pred_pc_fps, resume.split('/')[-2]+str('.pth').replace('.','_'+str(path)+'_'+str(non_uniform)+'_PRED_'+str(epoch)+'.'), exp_name)#prediction
                    plot_save_pcd(input, resume.split('/')[-2]+str('.pth').replace('.','_'+str(path)+'_'+str(non_uniform)+'_INPUT_'+str(epoch)+'.'), exp_name)#prediction
                    plot_save_pcd(gt, resume.split('/')[-2]+str('.pth').replace('.','_'+str(path)+'_'+str(non_uniform)+'_GT_'+str(epoch)+'.'), exp_name)#prediction
                    
                #print('File :: '+mesh_file.split('/')[-2]+' INPUT PC :: ', input.shape,' GT PC :: ', gt.shape,' PRED PC  :: ', pred_pc_fps.shape,' Centroid :: ',centroid.shape,' CD :: ',cd.item(),' HD :: ',hd,' p2f :: ',p2f_value.item(),' Total time :: ',end - start)

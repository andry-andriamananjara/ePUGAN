...
def plot_save_pcd(pcd, file, exp_name):
    ...
    img = draw_point_cloud(pcd, zrot=0 / 180.0 * np.pi, xrot=0 / 180.0 * np.pi, yrot=0 / 180.0 * np.pi,
                           diameter=4)
    img=(img*255).astype(np.uint8)
    image_save_path=os.path.join(image_save_dir,file_name+".jpeg")
...
def read_xyz(off_file):

    pcd = o3d.io.read_point_cloud(off_file)
...
def predict_from_h5():
    ...
    for epoch in [19, 29, 39, 49, 59, 69, 79, 89, 99]:
        ...
        if Path(ckpt_model).is_file() and resume.find(str(epoch))!=-1:
            print(ckpt_model)
            ...
            list_xyz = glob.glob(pathfile+'/*.xyz')
            #list_xyz = glob.glob('../MC_5k/Mydataset/PU_LiDAR/rectified_scans_local/xyz_file/*.xyz')

            #for path in range(3):
            for path in tqdm(range(len(list_xyz))):

                #print(list_xyz[path])
                filename  = list_xyz[path]
                input     = read_xyz(filename)
                print(filename)                
                if params['scale']=='arbitrary':
                    
                    alpha       = random.choice(list_alpha)
                    npoint      = input.shape[1]
                    ...
                    plot_save_pcd(pred_pc_fpsMI, pathfile+'/MI_'+filename.split('/')[-1].replace('.',str(epoch)+'.'), exp_name+'_xyz')#prediction
                ...
                plot_save_pcd(pred_pc_fps, pathfile+'/PRED_'+filename.split('/')[-1].replace('.',str(epoch)+'.'), exp_name+'_xyz')#prediction
                plot_save_pcd(input, pathfile+'/INPUT_'+filename.split('/')[-1].replace('.',str(epoch)+'.'), exp_name+'_xyz')#prediction
